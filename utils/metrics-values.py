...
from data.nef_mnist_dataset import MnistNeFDataset, ModelTransform
from networks.nano_gpt import GPT
from utils import get_default_device
from vector_quantize_pytorch import VectorQuantize
from einops import rearrange

from animation.animation_util import reconstruct_image_correct_and_not_luis_bs
...
dataset = MnistNeFDataset(
    os.path.join(data_root, "datasets", "mnist-nerfs"),
...
if not skip_gt:
    ...
    for i, (neural_field, label) in tqdm(
        enumerate(dataset), "Neural Field", total=dataset_length
    ):
...
if not skip_novel:
    ...
    with tqdm(
        total=len(top_k_options) * len(temperature_options), desc="Main Generation"
    ) as upper_pbar:

        for top_k in top_k_options:
            for temperature in temperature_options:
                upper_pbar.set_description(
                ...
                with tqdm(total=dataset_length, desc="Neural Field") as pbar:
                    for i in range(0, dataset_length, batch_size):
                        models = generate_neural_field(
                            model,
                            vq,
                            token_dict["SOS"],
                            [token_dict[str(label)] for _ in range(batch_size)],
                            device,
                            top_k=top_k,
                        ...
                        for j, mlp3d in enumerate(models):
                            if i + j >= dataset_length:
                                break
                            reconstructed = reconstruct_image_correct_and_not_luis_bs(
                                mlp3d
                            )
                ...
                torch.save(
                    imgs,
                    f"./models/metrics/mnist_novel_images_tk_{top_k}_temp_{temperature}.pt",
                )
                upper_pbar.update(1)
