...
class GlobalAutoencoder(nn.Module):
    def __init__(self,
        ...
        self.local_autoencoder = VanillaAutoencoder(input_dim, latent_dim_local, num_layers_enc_local, num_layers_dec_local, emb_dim_local, num_vec_local, local_latent_emb)
...
def construct_pos_encoder(input_dim, emb_dim, latent_dim, num_layers_enc, num_vec):
    ...
    for i in range(num_layers_enc - 1):
        dimensions_encoder.append(round(input_dim + emb_dim - (reduction_factor_input ) * (i + 1)/num_layers_enc))

    dimensions_encoder.append(latent_dim)

    return PositionEncoder(num_vec, emb_dim, dimensions_encoder)

def construct_pos_decoder(input_dim, output_dim, emb_dim, num_layers_dec, num_vec, with_upscaling=True):
    ...
    for i in range(num_layers_dec - 1):
        dimensions_decoder.append(round(input_dim + emb_dim - (increasing_factor_latent) * (i + 1)/num_layers_dec))
...
class VanillaAutoencoder(nn.Module):
    def __init__(self,
        ...
        for i in range(num_layers_dec - 1):
            dimensions_decoder.append(round(input_dim + local_latent_emb - (increasing_factor_latent) * (i + 1)/num_layers_dec))
            
        dimensions_decoder.append(output_dim)


        if local_latent_emb:
            self.decoder = PositionEncoder(num_vec, local_latent_emb, dimensions_decoder)
        else:
            self.decoder = VanillaDecoder(dimensions_decoder)
    ...
    def forward(self, x):
        ...
        if self.local_latent_emb:
            if self.with_upscaling:
                latent_upscaled = torch.cat((self.upscaling(latent[:, :, :-1]), latent[:, :, -1:]), dim=2)
