...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            print("Using manuel implementation of attention")
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.continuous_2_token = nn.Linear(config.input_size, config.vocab_size)
        self.nn_embedding = nn.Linear(config.vocab_size, config.n_embd)
    ...
    # just changed idx to x (we dont have an token embedding anymore)
    def forward(self, x, targets=None, offset=None, reduction="mean"):
        device = x.device
        b, t, x_emb = x.size()
        ...
        scaling_factor = torch.sqrt(torch.Tensor([t*x_emb]))
        scaling_factor.to(device)
        
        logits = self.continuous_2_token(x)
        prob = F.softmax(logits, dim=-1)
        tok = self.nn_embedding(prob)
