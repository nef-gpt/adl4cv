...
class Model(nn.Module):
    def __init__(
        ...
        # self.vq = ResidualVQ(dim=embedding_dim,
        #                      num_quantizers=1024, codebook_size=num_embeddings, decay=decay)
        if decay > 0.0:
    ...
    def forward(self, x):
        ...
        # quantized, codes, all_losses = self.vq(z.view(-1, embedding_dim))
        # loss = all_losses.sum()
        loss, quantized, perplexity, codes = self._vq_vae(z)
        quantized = quantized.reshape(z.shape)
        x_recon = self._decoder(quantized)

        codes = codes.reshape(b, -1)

        return loss, x_recon, perplexity, codes
