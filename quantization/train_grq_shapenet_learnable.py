...
if __name__ == "__main__":
    ...
    shapeNetData_normalized = ShapeNetDataset(
        "./datasets/plane_mlp_weights",
        transform=[AllWeights3D(), ZScore3D(means_neurons, 1)],
    )
    ...
    shapeNetData_normalized = ShapeNetDataset(
        ...
        transform=[
            AllWeights3D(),
            ZScore3D(means_neurons, 1),
            ZScore3D(mean_total, std_total),
        ],
    )

    main(
        shapeNetData_normalized,
        dims=[128],
        vocab_sizes=[128],
        batch_sizes=[256],
        threshold_ema_dead_codes=[0],
