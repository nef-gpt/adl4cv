...
def find_best_vq(
    input, kmean_iters, codebook_size=2**8 - 11, trials=25, training_iters=1000
):
    vq_lowest_loss = None
    lowest_loss = np.inf
    ...
    for _ in tqdm(range(trials)):
        ...
        vq = VectorQuantize(**vq_config).to("cuda")
...
def main():
    ...
    vq_path = os.path.join(
        dir_path, "adl4cv", "models", "vqs", "vq_mnist_with_all_5_conditioned_n_501.pt"
    )
    torch.save(
        {
    ...
    quantized_dataset_conditioned = MnistNeFDataset(
        ...
        type="pretrained",
        fixed_label=3,
        transform=QuantizeTransform(vq.to("cpu")),
    )
