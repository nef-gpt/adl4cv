...
from data.neural_field_datasets import DWSNetsDataset, FlattenMinMaxTransform, MnistNeFDataset, FlattenTransform, MinMaxTransform
...
# Config Training
training_config = naive_approach.Config()
...
def get_batch(split: str, n: int, block_size: int, dataset):
    ...
    x = torch.stack(
        [sample[i : i + block_size] for i in ix]
    )
    y = torch.stack(
        [sample[i + 1 : i + 1 + block_size] for i in ix]
    )
...
def do_type(type: str):
  ...
  for config in [big_model_config, small_model_config]:
    ...
    model_config = RegressionTransformerConfig(block_size=len(dataset[0][0]) - 1, **config)
    ...
    for n in n_values:
      ...
      if os.path.exists(f"./models/{detailed_folder}"):
        ...
        print(f"Training with n={n} type={type} and model_config={model_config}")
        get_model_batch = lambda split: get_batch(split, n, model_config.block_size, dataset)
        training_config.detailed_folder = detailed_folder
        training_config.wandb_run_name = f"n_{n}_type_{type}_model_{big_label}"
        naive_approach.train(get_model_batch, training_config, model_config)
      ...
      try:
        for idx in range(0, n):
          ...
          visualize_learning_process(idx, training_config.max_iters, detailed_folder, video_name,  dataset_kwargs=kwargs, regression_config=model_config)             
      # catch all errors
